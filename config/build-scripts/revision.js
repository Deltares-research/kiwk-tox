...
function revision() {
  return revisionHash()
    ...
    .on('end', () =>
      console.log(
        `Revisioned files in ${path.relative(projectDir, workingDir)}`
      )
    );
}
...
function revisionHash() {
  return gulp
    .src([
      // revision files matching these patterns
      `${workingDir}/**/*.{css,css.map,js,js.map}`,
      `${workingDir}/**/*.{gif,ico,jpg,png,svg,webp}`,
      `${workingDir}/**/*.{woff,woff2,eot,ttf}`,
    ])
...
function revisionReplace() {
  ...
  return gulp
    ...
    .pipe(
      revReplace({
        // replace referenced to revisioned files
        manifest: gulp.src(manifestPath), // which are listed in the revision manifest
        replaceInExtensions: ['.js', '.css', '.html', '.webmanifest', '.xml'],
      })
    ) //
